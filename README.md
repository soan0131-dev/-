...
- **전직원 현황판** (`/employees`): 기본 인적사항, 본부·부서, 재직상태, 연락처, 직급/자격,
  ...
  이력이 함께 삭제됩니다.
- **입사자 일괄 등록** (`/employees/bulk-upload`, HR/관리자 전용): 엑셀(CSV) 템플릿을
  다운로드해 여러 명을 한 번에 작성한 뒤 업로드하면, 행마다 신규 직원과 입사 케이스가 자동으로
  생성됩니다(한 번에 최대 1,000건). 행별로 성공/실패 결과와 실패 사유(필수값 누락, 부서코드
  오류, 직급/자격 등 값 인식 불가, 날짜 형식 오류, 사번/이메일 중복 등)를 화면에서 바로
  확인할 수 있습니다.
