...
- **전직원 현황판** (`/employees`): 기본 인적사항, 본/지사·부서, 재직상태, 연락처, 직급/자격,
  보유자격(KICPA/AICPA 등)을 본지사/부서/재직상태/자격으로 필터링해 조회. 입퇴사 케이스가
  ...
  이력이 함께 삭제됩니다.
- **입퇴사 케이스 생성** (`/cases/new`, HR/관리자 전용): 입사자는 본/지사·부서(종속 드롭다운),
  직급(선택 시 파트너 유형 조건부 노출), 자격(중복 선택), 제출서류 체크리스트 유형 등 신규
  직원 정보를 입력해 케이스를 시작합니다. 퇴사자는 이름/사번으로 검색해 전직원 중에서 선택하고,
