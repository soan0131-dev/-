...
- **전직원 현황판** (`/employees`): 기본 인적사항, 본부·부서, 재직상태, 연락처, 직급/자격,
  보유자격(KICPA/AICPA 등)을 본부/부서/재직상태/자격으로 필터링해 조회. 입퇴사 케이스가
  완료되면 이 마스터 데이터가 자동 갱신됩니다. 현재 필터 조건을 반영한 **엑셀(CSV) 다운로드**를
  지원해 사용자가 다운로드한 파일에서 자유롭게 추가 필터링·피벗할 수 있습니다. 직원 상세
  화면(HR/관리자 전용)에서 **직원 삭제**도 가능하며, 삭제 시 관련 입퇴사 케이스·서류·결재
  이력이 함께 삭제됩니다.
- **입퇴사 케이스 생성** (`/cases/new`, HR/관리자 전용): 입사자는 본부·부서(종속 드롭다운),
  계약형태(정규직/계약직), 직급(선택 시 파트너 유형 조건부 노출), 자격(중복 선택), 생년월일,
  제출서류 체크리스트 유형 등 신규 직원 정보를 입력해 케이스를 시작합니다. 퇴사자는 이름/사번으로 검색해 전직원 중에서 선택하고,
  퇴사 사유를 개인사유/이직/권고사직/기타 중 선택(기타 선택 시 직접 입력)해 케이스를 시작합니다.
...
- **입사 제출서류 체크리스트/업로드/다운로드**: 입사 케이스 생성 시 "준전문직 /
  전문직 / 계약직" 중 체크리스트 유형을 선택하면, 실제 회사 양식(성현회계법인
